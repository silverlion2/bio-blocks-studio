...
- While dragging within the same section, a faint floating preview moves to the intended logical grid cell before mouseup. The preview is absolutely positioned and does not participate in CSS grid layout, so nearby blocks do not shrink, disappear, or get reflowed just because the preview moved.
