...
- **浏览器草稿恢复**
  有效但未发布的修改会自动备份在当前浏览器。刷新后可选择恢复、导出备份或放弃；远端保存失败不会关闭编辑弹窗或清除草稿。自动草稿不会持久化版本访问码。
...
若 Blob 未配置或远端保存失败，后台会明确显示“尚未发布”，并保留当前浏览器草稿。刷新后必须先恢复或放弃检测到的草稿，才能继续保存。浏览器草稿通过同一份 `SiteConfig` 校验，但会剥离版本访问码；恢复时沿用服务器当前加载的访问码。
